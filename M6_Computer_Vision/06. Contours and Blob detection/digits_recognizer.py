...
def get_surrounding_box(img):

    orig = img.copy()

    kernel_dim = int(max(img.shape) * 0.02525) if int(max(img.shape) * 0.02525) % 2 != 0 else int(max(img.shape) * 0.02525) + 1
    st = kernel_dim * 0.0495
    
    box = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    gauss_kernel = (kernel_dim, kernel_dim)
    box = cv2.GaussianBlur(box, gauss_kernel, st)
    ...
    cv2.drawContours(orig, [box_contours[0]], 0, BOX_COLOR, 2)
